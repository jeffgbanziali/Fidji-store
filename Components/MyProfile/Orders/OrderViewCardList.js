...
const OrderViewCardList = ({ formattedOrder }) => {



    const orderRenderItems = ({ item }) => {
        return (
            <View style={styles.itemContainer}>
                <Image source={{ uri: item.image }} style={styles.itemImage} />
                <View style={styles.itemDetails}>
                    <Text style={styles.itemName}>{item.name}</Text>
                    <Text style={styles.itemSize}> {item.size} / {item.price.toFixed(2)}€</Text>
                </View>
                <View style={{
                    width: 20,
                    height: 100,
...
export default OrderViewCardList